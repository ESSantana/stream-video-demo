...
resource "aws_lambda_permission" "upload_video_lambda_permission" {
  ...
  source_arn = "arn:aws:execute-api:${var.aws_region}:${var.accountId}:${aws_api_gateway_rest_api.stream_video_api.id}/*/${aws_api_gateway_method.upload_video_method.http_method}${aws_api_gateway_resource.upload_video_resource.path}"
}
